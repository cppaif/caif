...
//------------------------------------------------------------------------------
// CAIF - AI Framework
// Write-only RAII float view over a host-backed CAIF_DeviceTensor. The FP32
// path is zero-copy (the view writes directly into the tensor). The FP16 /
// BF16 path allocates an FP32 scratch buffer at construction and down-casts
// it back into the target tensor at destruction.
//------------------------------------------------------------------------------
#pragma once
...
class CAIF_HostFloatWriteView:public CAIF_Base
{
  public:
    explicit CAIF_HostFloatWriteView(CAIF_DeviceTensor &t):_tensor(t),
                                                            _ptr(nullptr),
                                                            _scratch(),
                                                            ...
                                                            _needs_writeback(false)
    {
      ...
      {
        ScratchMutable().resize(Size());
        SetPtr(ScratchMutable().data());
        SetNeedsWriteback(true);
      }
    }

    ~CAIF_HostFloatWriteView()
    {
