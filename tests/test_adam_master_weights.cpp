...
constexpr uint32_t g_param_rows=8;
constexpr uint32_t g_param_cols=16;
constexpr float g_adam_lr=0.01f;
...
class TestParamLayer:public CAIF_DeviceLayer
{
  public:
    ...
    {
      const std::vector<uint32_t> shape={g_param_rows,g_param_cols};
...
std::vector<float> MakeInitialParam(const size_t n,const uint32_t seed)
{
  std::vector<float> out(n);
  for(size_t i=0;i<n;++i)
  {
    const float sgn=(((i+seed)%2)==0)?1.0f:-1.0f;
    out[i]=sgn*0.1f*(1.0f+0.01f*static_cast<float>((i+seed)%11));
  }
  return out;
}

std::vector<float> MakeGradPattern(const size_t n)
{
  std::vector<float> out(n);
  for(size_t i=0;i<n;++i)
  {
    const float sgn=((i%2)==0)?1.0f:-1.0f;
    out[i]=sgn*0.1f*(1.0f+0.01f*static_cast<float>(i%7));
  }
  return out;
}

std::unique_ptr<CAIF_DeviceNetwork> BuildSingleParamNet(
  CAIF_DataType::CAIF_DataType_e param_dtype,
  CAIF_CudaStream &stream)
{
  auto net=std::make_unique<CAIF_DeviceNetwork>(stream);
  auto layer=std::make_unique<TestParamLayer>(stream,param_dtype);
  ...
  const size_t n=static_cast<size_t>(g_param_rows)*g_param_cols;
  CopyFp32HostToDeviceTensor(MakeInitialParam(n,7),layer->ParameterTensor(0),stream);
...
void RunAdamSteps(CAIF_DeviceNetwork &net,CAIF_CudaStream &stream,int steps)
{
  const size_t n=static_cast<size_t>(g_param_rows)*g_param_cols;
...
void TestAdamFp32FastPath()
{
  try
  {
    ...
    if(ok==false)
    {
      ISE_Out::Out()<<"  Fp32 finite="<<finite<<" peak="<<peak
                    <<" dtype_ok="<<dtype_ok<<"\n";
    }
    ReportResult("AdamMaster::Fp32FastPath",ok);
  }
  CAIF_TEST_CATCH_BLOCK("AdamMaster::Fp32FastPath")
}

void TestAdamFp16MasterPath()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    if(ok==false)
    {
      ISE_Out::Out()<<"  Fp16 dtype_is_fp16="<<dtype_is_fp16
                    <<" dtype_preserved="<<dtype_preserved
                    <<" finite="<<finite
                    <<" peak_delta="<<peak<<"\n";
    }
    ReportResult("AdamMaster::Fp16MasterPath",ok);
  }
  CAIF_TEST_CATCH_BLOCK("AdamMaster::Fp16MasterPath")
}

void TestAdamBf16MasterPath()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    if(ok==false)
    {
      ISE_Out::Out()<<"  Bf16 dtype_is_bf16="<<dtype_is_bf16
                    <<" dtype_preserved="<<dtype_preserved
                    <<" finite="<<finite
                    <<" peak_delta="<<peak<<"\n";
    }
    ReportResult("AdamMaster::Bf16MasterPath",ok);
  }
...
void TestAdamBf16MatchesFp32Over20Steps()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    fp32_net->InitializeAdam(g_adam_lr);
    bf16_net->InitializeAdam(g_adam_lr);

    RunAdamSteps(*fp32_net,stream,20);
    RunAdamSteps(*bf16_net,stream,20);
    ...
    const bool close=diff<2e-2f;
    if(close==false)
    {
      ISE_Out::Out()<<"  Bf16VsFp32 peak_diff="<<diff<<"\n";
    }
    ReportResult("AdamMaster::Bf16MatchesFp32Over20Steps",close);
  }
