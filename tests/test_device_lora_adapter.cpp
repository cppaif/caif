...
constexpr uint32_t g_caif_lora_test_input_dim=16;
constexpr uint32_t g_caif_lora_test_output_dim=8;
constexpr uint32_t g_caif_lora_test_batch=2;
constexpr uint32_t g_caif_lora_test_rank=4;
constexpr float g_caif_lora_test_alpha=8.0f;
constexpr float g_caif_lora_test_match_tol=1e-4f;

//------------------------------------------------------------------------------
// LoRA adapter correctness and dtype-sweep tests.
//------------------------------------------------------------------------------
class CAIF_LoRAAdapterTests
{
  public:
    static void RunAll();
  ...
  private:
    static CAIF_DeviceLoRAAdapter<float,float> MakeLoRA(CAIF_CudaStream &stream,
                                                        uint32_t seed=42);
...
CAIF_DeviceLoRAAdapter<float,float> CAIF_LoRAAdapterTests::MakeLoRA(CAIF_CudaStream &stream,
                                                                      const uint32_t seed)
{
  auto frozen=std::make_unique<CAIF_DeviceFrozenLinear<float,float>>(g_caif_lora_test_input_dim,
                                                                      g_caif_lora_test_output_dim,
                                                                      stream);
  // Load random weights
  std::mt19937 gen(seed);
  std::uniform_real_distribution<float> dist(-0.5f,0.5f);
  const size_t count=static_cast<size_t>(g_caif_lora_test_input_dim)*g_caif_lora_test_output_dim;
  ...
  CAIF_DeviceTensor weight=CAIF_DeviceTensor::FromHostData(
                             w.data(),
                             {g_caif_lora_test_input_dim,g_caif_lora_test_output_dim},
                             stream);
  frozen->LoadFromTensor(std::move(weight));

  CAIF_DeviceLoRAAdapterConfig config(g_caif_lora_test_rank,
                                      g_caif_lora_test_alpha,
                                      g_caif_lora_test_input_dim,
                                      g_caif_lora_test_output_dim);

  return CAIF_DeviceLoRAAdapter<float,float>(config,std::move(frozen),stream,seed+1);
}

std::vector<float> CAIF_LoRAAdapterTests::MakeRandomInput(const uint32_t n,
                                                           const uint32_t dim,
                                                           const uint32_t seed)
{
  std::mt19937 gen(seed);
...
template<typename T>
CAIF_DataType::CAIF_DataType_e CAIF_LoRAAdapterTests::DtypeFromCpp()
{
  return CAIF_DataType::CAIF_DataType_e::Float32;
}

template<>
CAIF_DataType::CAIF_DataType_e CAIF_LoRAAdapterTests::DtypeFromCpp<__half>()
{
  return CAIF_DataType::CAIF_DataType_e::Float16;
}

template<>
CAIF_DataType::CAIF_DataType_e CAIF_LoRAAdapterTests::DtypeFromCpp<__nv_bfloat16>()
{
...
// is what bit the dispatch sweep).
//------------------------------------------------------------------------------
template<typename ComputeT,typename StorageT>
void CAIF_LoRAAdapterTests::TestLoRACellForwardBackward(const char *cell_name,const uint32_t seed)
{
  try
  {
    ...
    auto frozen=std::make_unique<CAIF_DeviceFrozenLinear<ComputeT,StorageT>>(
                  ...
                  g_caif_lora_test_output_dim,
                  stream);
    std::vector<float> w(static_cast<size_t>(g_caif_lora_test_input_dim)*g_caif_lora_test_output_dim);
    std::mt19937 gen(seed);
    std::uniform_real_distribution<float> dist(-0.5f,0.5f);
    ...
    auto host_input=MakeRandomInput(g_caif_lora_test_batch,g_caif_lora_test_input_dim,seed*7u+1u);
    CAIF_DeviceTensor input_fp32=CAIF_DeviceTensor::FromHostData(
                                   host_input.data(),
                                   {g_caif_lora_test_batch,g_caif_lora_test_input_dim},
                                   stream);
    ...
    if(passed==true)
    {
      std::vector<float> grad_data(static_cast<size_t>(g_caif_lora_test_batch)*
                                   g_caif_lora_test_output_dim,
                                   1.0f);
      CAIF_DeviceTensor grad_fp32=CAIF_DeviceTensor::FromHostData(
                                    grad_data.data(),
                                    {g_caif_lora_test_batch,g_caif_lora_test_output_dim},
                                    stream);
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestForwardShape()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    auto host_input=MakeRandomInput(g_caif_lora_test_batch,g_caif_lora_test_input_dim);
    ...
    if(shape.size()!=2 ||
       shape[0]!=g_caif_lora_test_batch ||
       shape[1]!=g_caif_lora_test_output_dim)
    {
      ISE_Out::Out()<<"  Shape mismatch\n";
      passed=false;
    }
    CAIF_TestHarness::Report("LoRAAdapter::ForwardShape",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestForwardFinite()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    CAIF_TestHarness::Report("LoRAAdapter::ForwardFinite",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestZeroInitBMatchesBase()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    CAIF_DeviceFrozenLinear<float,float> frozen_copy(g_caif_lora_test_input_dim,
                                                     g_caif_lora_test_output_dim,
                                                     stream);
    CAIF_DeviceTensor w_copy=CAIF_DeviceTensor::FromHostData(
                               w.data(),
                               {g_caif_lora_test_input_dim,g_caif_lora_test_output_dim},
                               stream);
    ...
    ctx.SetTraining(false);
    ctx.SetPass(CAIF_RunContext::Pass_e::Forward_e);
    auto host_input=MakeRandomInput(g_caif_lora_test_batch,g_caif_lora_test_input_dim,777);
    CAIF_DeviceTensor input=CAIF_DeviceTensor::FromHostData(
                              host_input.data(),
                              {g_caif_lora_test_batch,g_caif_lora_test_input_dim},
                              stream);
    ...
    for(size_t i=0;i<h_base.TotalElements();++i)
    {
      ...
      if(diff>g_caif_lora_test_match_tol)
      {
    ...
    CAIF_TestHarness::Report("LoRAAdapter::ZeroInitBMatchesBase",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestParameterTensorCount()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    if(passed==false)
    {
      ISE_Out::Out()<<"  Expected 2, got "
                    <<lora.ParameterTensorCount()
                    <<"\n";
    }
    CAIF_TestHarness::Report("LoRAAdapter::ParameterTensorCount",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestTotalParameterCount()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    const size_t expected=static_cast<size_t>(g_caif_lora_test_rank)*g_caif_lora_test_input_dim+
                          static_cast<size_t>(g_caif_lora_test_output_dim)*g_caif_lora_test_rank;
    ...
    if(passed==false)
    {
      ISE_Out::Out()<<"  Expected "
                    <<expected
                    <<", got "
                    <<lora.TotalParameterCount()
                    <<"\n";
    }
    CAIF_TestHarness::Report("LoRAAdapter::TotalParameterCount",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestParameterNames()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    if(names.size()!=2)
    {
      ISE_Out::Out()<<"  Expected 2 names, got "
                    <<names.size()
                    <<"\n";
      passed=false;
    }
    else
    {
      ...
      if(names[1]!="layer.lora_b.weight")
      {
        ISE_Out::Out()<<"  names[1]="
                      <<names[1]
                      <<" expected layer.lora_b.weight\n";
        passed=false;
      }
    }
    CAIF_TestHarness::Report("LoRAAdapter::ParameterNames",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestBackwardFiniteInputGradients()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    if(shape.size()!=2 ||
       shape[0]!=g_caif_lora_test_batch ||
       shape[1]!=g_caif_lora_test_input_dim)
    {
    ...
    CAIF_TestHarness::Report("LoRAAdapter::BackwardFiniteInputGradients",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestBackwardNonZeroLoRAGradients()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    const size_t b_count=static_cast<size_t>(g_caif_lora_test_output_dim)*g_caif_lora_test_rank;
    ...
    for(size_t p=0;p<lora.ParameterTensorCount();++p)
    {
      ...
      for(size_t i=0;i<h_grad.TotalElements();++i)
      {
        if(std::isfinite(h_grad.Data()[i])==false)
        {
          ISE_Out::Out()<<"  Non-finite LoRA grad["
                        <<p
                        <<"] at "
                        <<i
                        <<"\n";
      ...
      {
        ISE_Out::Out()<<"  LoRA grad["
                      <<p
                      <<"] is all zeros\n";
        passed=false;
      }
    }
    CAIF_TestHarness::Report("LoRAAdapter::BackwardNonZeroLoRAGradients",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestZeroGradients()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    CAIF_TestHarness::Report("LoRAAdapter::ZeroGradients",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestDescription()
{
  try
  {
    CAIF_CudaStream stream;
    ...
    {
      ISE_Out::Out()<<"  Missing 'LoRA': "
                    <<desc
                    <<"\n";
      passed=false;
    }
    if(desc.find(std::to_string(g_caif_lora_test_rank))==std::string::npos)
    {
      ISE_Out::Out()<<"  Missing rank: "
                    <<desc
                    <<"\n";
      passed=false;
    }
    CAIF_TestHarness::Report("LoRAAdapter::Description",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::Test3DInputForwardShape()
{
  try
  {
    ...
    auto host_input=MakeRandomInput(batch*seq_len,g_caif_lora_test_input_dim);
    CAIF_DeviceTensor input=CAIF_DeviceTensor::FromHostData(
                              host_input.data(),
                              {batch,seq_len,g_caif_lora_test_input_dim},
                              stream);
    ctx.SetTraining(false);
    ctx.SetPass(CAIF_RunContext::Pass_e::Forward_e);
    CAIF_DeviceTensor output=lora.Forward(input,ctx);
    ...
    if(shape.size()!=3 ||
       shape[0]!=batch ||
       shape[1]!=seq_len ||
       shape[2]!=g_caif_lora_test_output_dim)
    {
      ISE_Out::Out()<<"  Shape mismatch\n";
      passed=false;
    }
    CAIF_TestHarness::Report("LoRAAdapter::3DInputForwardShape",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestLoRAWithFP16Frozen()
{
  try
  {
    CAIF_CudaStream stream;

    auto frozen=std::make_unique<CAIF_DeviceFrozenLinear<float,__half>>(g_caif_lora_test_input_dim,
                                                                         g_caif_lora_test_output_dim,
                                                                         stream);
    std::vector<float> w(static_cast<size_t>(g_caif_lora_test_input_dim)*g_caif_lora_test_output_dim);
    std::mt19937 gen(11);
    ...
    CAIF_TestHarness::Report("LoRAAdapter::LoRAWithFP16Frozen",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestLoRAWithINT4Frozen()
{
  try
  {
    ...
    CAIF_TestHarness::Report("LoRAAdapter::LoRAWithINT4Frozen",passed);
  }
...
//------------------------------------------------------------------------------
void CAIF_LoRAAdapterTests::TestLoRAInPreNormBlock()
{
  try
  {
    const uint32_t dim=g_caif_lora_test_input_dim;
    CAIF_CudaStream stream;
    ...
    CAIF_DeviceLoRAAdapterConfig config(g_caif_lora_test_rank,g_caif_lora_test_alpha,dim,dim);
    ...
    {
      ISE_Out::Out()<<"  Expected 3 params, got "
                    <<block.ParameterTensorCount()
                    <<"\n";
      passed=false;
    }

    CAIF_TestHarness::Report("LoRAAdapter::LoRAInPreNormBlock",passed);
  }
  CAIF_TEST_CATCH_BLOCK("LoRAAdapter::LoRAInPreNormBlock")
}

void CAIF_LoRAAdapterTests::RunAll()
{
...
{
#ifdef USE_CAIF_CUDA
  instance::CAIF_LoRAAdapterTests::RunAll();
