...
CAIF_DeviceMoERouter<ComputeT,StorageT>::CAIF_DeviceMoERouter(const Config_t &config,CAIF_CudaStream &stream)
  ...
  ,_config(config)
{
  try
  {
    ...
    if(Config().gating_kind==CAIF_DeviceMoELayerFactory::GatingKind_e::SigmoidNoauxTc_e&&
       Config().routing_type!=RoutingType_e::TopK)
    {
    ...
    SetWRouter(CAIF_DeviceTensor::Uninitialized({Config().input_dim,Config().num_experts},
                                                stream,
                                                sd));
    SetGradWRouter(CAIF_DeviceTensor::Zeros({Config().input_dim,Config().num_experts},
                                            stream,
                                            sd));

    if(Config().use_bias==true)
    {
      SetBRouter(CAIF_DeviceTensor::Zeros({Config().num_experts},stream,sd));
      SetGradBRouter(CAIF_DeviceTensor::Zeros({Config().num_experts},stream,sd));
    }
    ...
    std::mt19937 gen(rd());
    float scale=std::sqrt(2.0f/static_cast<float>(Config().input_dim+Config().num_experts));
    std::normal_distribution<float> dist(0.0f,scale);

    std::vector<float> data(Config().input_dim*Config().num_experts);
...
CAIF_DeviceMoERouter<ComputeT,StorageT>::RouterOutput_t CAIF_DeviceMoERouter<ComputeT,StorageT>::Route(const CAIF_DeviceTensor &input,CAIF_RunContext &ctx)
{
  try
  {
    ...
    if(flat_input.Shape()[1]!=Config().input_dim)
    {
    ...
    CAIF_DeviceTensor logits=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().num_experts},Stream(),sd);
    ...
    // per-token clone.
    if(Config().use_bias==true&&
       Config().gating_kind==CAIF_DeviceMoELayerFactory::GatingKind_e::SoftmaxTopK_e)
    {
      CAIF_Ops::AddBias(logits,BRouter(),logits);
    }

    // Add noise during training for exploration
    if(ctx.Training()==true&&Config().noise_std>0.0f)
    {
      // Generate noise on host (fp32), upload to fp32 staging, cast to StorageT.
      std::vector<float> noise_data(num_tokens*Config().num_experts);
      std::random_device rd;
      std::mt19937 gen(rd());
      std::normal_distribution<float> dist(0.0f,Config().noise_std);
    ...
    CAIF_DeviceTensor probs=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().num_experts},Stream(),sd);

    if(Config().gating_kind==CAIF_DeviceMoELayerFactory::GatingKind_e::SigmoidNoauxTc_e)
    {
      CAIF_Ops::Sigmoid(logits,probs);
    }
    else if(Config().routing_type==RoutingType_e::ExpertChoice)
    {
      // Expert Choice: softmax over tokens for each expert
      // Transpose logits to [num_experts, num_tokens], softmax, transpose back
      CAIF_DeviceTensor logits_t=CAIF_DeviceTensor::Uninitialized({Config().num_experts,num_tokens},Stream(),sd);
      CAIF_Ops::Transpose(logits,logits_t);
      CAIF_DeviceTensor probs_t=CAIF_DeviceTensor::Uninitialized({Config().num_experts,num_tokens},Stream(),sd);
    ...
    switch(Config().routing_type)
    {
      case RoutingType_e::TopK:
      {
        ...
        CAIF_DeviceTensor weights=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().top_k},Stream(),sd);
           ...
           Config().use_bias==true)
        {
          ...
          CAIF_Ops::TopK(selection,Config().top_k,indices,weights);
          CAIF_Ops::GatherTopKValues(probs,indices,weights);
        }
        else
        {
          CAIF_Ops::TopK(probs,Config().top_k,indices,weights);
        }
        ...
        if(Config().norm_topk_prob==true)
        {
          CAIF_Ops::NormalizeRows(weights,weights);
        }

        // SigmoidNoauxTc applies a multiplicative routed_scaling_factor
        // post-normalise; SoftmaxTopK doesn't (mathematically a no-op at
        // 1.0f, gated to skip the kernel launch).
        if(Config().gating_kind==CAIF_DeviceMoELayerFactory::GatingKind_e::SigmoidNoauxTc_e&&
           Config().routed_scaling_factor!=1.0f)
        {
          CAIF_Ops::Scale(weights,Config().routed_scaling_factor);
        }
      ...
      case RoutingType_e::ExpertChoice:
      {
        ...
        const uint32_t capacity=std::max(1u,(num_tokens*Config().top_k)/Config().num_experts);
        ...
        CAIF_DeviceTensor token_indices=CAIF_DeviceTensor::Uninitialized(
                                          {Config().num_experts,capacity},
                                          Stream(),
                                          CAIF_DataType::CAIF_DataType_e::Int32);
        CAIF_DeviceTensor token_weights=CAIF_DeviceTensor::Uninitialized({Config().num_experts,capacity},Stream(),sd);
      ...
      case RoutingType_e::Soft:
      {
        ...
        CAIF_DeviceTensor indices=CAIF_DeviceTensor::Uninitialized(
                                    ...
                                    Stream(),
                                    CAIF_DataType::CAIF_DataType_e::Int32);
        std::vector<int32_t> idx_data(num_tokens*Config().num_experts);
        for(uint32_t t=0;t<num_tokens;++t)
        {
          for(uint32_t e=0;e<Config().num_experts;++e)
          {
            idx_data[t*Config().num_experts+e]=static_cast<int32_t>(e);
          }
        }
  ...
  CAIF_RunContext &ctx)
{
  try
  {
    ...
    if(Config().gating_kind==CAIF_DeviceMoELayerFactory::GatingKind_e::SigmoidNoauxTc_e)
    {
      ...
      const uint32_t top_k=Config().top_k;
      ...
      if(Config().routed_scaling_factor!=1.0f)
      {
        CAIF_Ops::Scale(GradTopkPreviewMut(),Config().routed_scaling_factor);
      }

      CAIF_DeviceTensor grad_probs=CAIF_DeviceTensor::Zeros({num_tokens,Config().num_experts},Stream(),bsd);
      CAIF_Ops::ScatterAdd(GradTopkPreview(),CachedIndices(),grad_probs);

      CAIF_DeviceTensor grad_logits=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().num_experts},
                                                                     Stream(),
                                                                     bsd);
      ...
      CAIF_DeviceTensor grad_input=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().input_dim},Stream(),bsd);
    ...
    const CAIF_DataType::CAIF_DataType_e bsd=StorageDtype();
    const bool topk_jacobian=(Config().routing_type==RoutingType_e::TopK&&
    ...
    CAIF_DeviceTensor grad_probs=
      CAIF_DeviceTensor::Zeros({num_tokens,Config().num_experts},Stream(),bsd);
    ...
    CAIF_DeviceTensor grad_logits=CAIF_DeviceTensor::Uninitialized({num_tokens,Config().num_experts},Stream(),bsd);
    ...
    CAIF_DeviceTensor grad_w_batch=
      CAIF_DeviceTensor::Uninitialized({Config().input_dim,Config().num_experts},Stream(),bsd);
    CAIF_Ops::MatMulTransposeA(CachedInput(),grad_logits,grad_w_batch,ctx,cdt);
    CAIF_Ops::Add(GradWRouter(),grad_w_batch,GradWRouterMut());
    ...
    if(Config().use_bias==true)
    {
      ...
      CAIF_DeviceTensor grad_b_batch=CAIF_DeviceTensor::Uninitialized({Config().num_experts},Stream(),bsd);
...
CAIF_DeviceTensor &CAIF_DeviceMoERouter<ComputeT,StorageT>::ParameterTensor(size_t index)
{
  ...
  if(Config().use_bias==true&&index==1)
  {
    return BRouterMut();
  }
...
{
  size_t count=Config().input_dim*Config().num_experts;  // w_router
  if(Config().use_bias==true)
  {
    count+=Config().num_experts;  // b_router
  }
  return count;
}
...
std::string CAIF_DeviceMoERouter<ComputeT,StorageT>::Description()const
{
  ...
  desc+=std::to_string(Config().input_dim)+"->"+std::to_string(Config().num_experts);
  desc+=",top_k="+std::to_string(Config().top_k);
  if(Config().use_bias==true)
  {
    desc+=",bias";
  }
  ...
  {
    desc+=",noise="+std::to_string(Config().noise_std);
  }
  desc+="]";
  return desc;
}
                                                                ...
                                                                float bias_magnitude)
{
  try
  {
    if(expert_index>=Config().num_experts)
    {
    ...
    std::vector<float> w_zero(Config().input_dim*Config().num_experts,0.0f);
    WRouterMut().CopyFromHostFp32(w_zero.data(),w_zero.size());

    // Bias: one-hot with `bias_magnitude` at `expert_index`, zero
    // elsewhere.
    std::vector<float> b_data(Config().num_experts,0.0f);
...
void CAIF_DeviceMoERouter<ComputeT,StorageT>::LoadBRouter(CAIF_DeviceTensor &&b)
{
  try
  {
    ...
    if(shape.size()!=1||shape[0]!=Config().num_experts)
    {
